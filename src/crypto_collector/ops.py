...
def build_health_report(
    *,
    ...
    # Latest archive-offload report. The report used to be discarded after a
    # one-line job log entry, so a growing stuck_unaccounted cohort was invisible
    # behind "all jobs success" (2026-07-04 audit: 14,211 runs / ~95 GB hid for a
    # week). A missing or unreadable file is NOT a finding — the file only exists
    # once an offload has run on post-deploy code, and alerting on absence would
    # recreate the alert-noise class PR #20 killed. Staleness is surfaced as
    ...
    if offload_payload is not None:
        ...
        stuck_raw = offload_payload.get("stuck_unaccounted_count")
        stuck_count = (
            int(stuck_raw)
            if isinstance(stuck_raw, (int, float)) and not isinstance(stuck_raw, bool)
            else None
        )
        ...
        offload_summary = {
            ...
            "stuck_unaccounted_count": stuck_count if stuck_count is not None else stuck_raw,
        ...
        # as the baseline (local health invocation config), so only NEW orphans —
        # a promoter/scorer stalling again — fire the advisory finding. Reset the
        # baseline to 0 once the owner clears the cohort.
        if stuck_count is not None and stuck_count > stuck_unaccounted_baseline:
