...
| Liquidations | Bybit BTC/ETH/SOL; OKX all swaps | **B** Bybit (clean, not gap-proof); **C** OKX until the v12 verdict (PR open) is merged, deployed and the history re-scored, then **B** | Bybit 2026-09-08; OKX capture 2026-08-25 | 2 d / 16 d | raw only; OKX: use `received_at` and per-product order (venue-delayed batches) |
...
| 28 | okx-swap-liquidations | OKX | all swaps | liquidations | WS, day segments | raw only | **C** capture-complete; the pre-v12 verdict was wrong for this feed (venue-delayed batches, 340 products). STANDARDS v12 (`replay_liquidations_run`, PR open) gates on receipt order and records the lag; **B** once merged, deployed and re-scored. Use `received_at` and per-product order |
